			...
			function processPlayerList(data) {
				...
				function nextPlayer() {
					...
					function processPlayerHistory(data) {
						...
						var weekPoints = Array(maxWeek + 1).fill(null, 1, maxWeek + 1);
						...
						for (var week = 1; week <= maxWeek; week++) {
							...
							$.each(weekData, (i,wd) => {
								if (weekPoints[week] === null) weekPoints[week] = 0;
								if (totalPoints === null) totalPoints = 0;
								weekPoints[week] += wd.total_points;
								totalPoints += wd.total_points;
							});
						}
						...
						$('<td></td>').appendTo(outputRow).text(totalPoints === null ? '' : totalPoints);
