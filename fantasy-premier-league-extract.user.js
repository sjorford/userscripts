	...
	$(function() {
		...
		function extract() {
			...
			function processPlayerList(data) {
				...
				var maxWeek = Math.max(...data.events.filter(week => week.finished == true).map(week => week.id));
				...
				function nextPlayer() {
					...
					function processPlayerHistory(data) {
						...
						for (var week = 1; week <= maxWeek; week++) {
							...
							$('<td></td>').appendTo(outputRow).text(weekData.length == 0 ? '' : weekData[0].total_points);
						}
