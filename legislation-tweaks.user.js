...
$(function() {
	...
	$('#viewLegSnippet *').contents().filter((i,e) => e.nodeType == 3).each((i,e) => {
		var match;
		while (match = e.nodeValue.match(/^(.*?)(?<!\d)(\d{4}|\d+ (electoral )?wards|\d+ divisions|is (\d|one|two|three|four|five|six))(?!\d)(.*)$/s)) {
			var before = document.createTextNode(match[1]);
			var after  = document.createTextNode(match[5]);
